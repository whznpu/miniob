...
RC Table::create(const char *path, const char *name, const char *base_dir,
  ...
  close(fd);
  // 使用 table_name.overflow 记录一个表的溢出数据
  ...
  base_dir_ = base_dir;
  // overflow_file_ =fopen(overflow_file_)
  clog_manager_ = clog_manager;
  LOG_INFO("Successfully create table %s:%s", base_dir, name);
  return rc;
}

RC Table::destory(const char *base_dir) {
  RC rc = sync();  // 刷新所有脏页
  ...
  // 此时用cout语句是无效的 要使用log debugg方法
  LOG_DEBUG("the path is %s", path.c_str());
  ...
  //   LOG_ERROR("Failed to remove meta data file=%s,errno=%d
  //   path=%s",meta_file.c_str(),errno,path.c_str()); return RC::GENERIC_ERROR;
  // }
  // 删除data
  std::string data_file = table_data_file(base_dir, name());
  LOG_DEBUG("path is %s", data_file.c_str());
  if (unlink(data_file.c_str()) != 0) {
    LOG_ERROR("Failed to remove data=%s,errno=%d", data_file.c_str(), errno);
    return RC::GENERIC_ERROR;
  }
  rc = BufferPoolManager::instance().close_file(data_file.c_str());
  ...
  for (int i = 0; i < index_num; i++) {
    ...
    if (unlink(index_file.c_str()) != 0) {
      LOG_ERROR("Failed to remove index_file=%s,errno=%d", index_file.c_str(),
                errno);
      return RC::GENERIC_ERROR;
    }
  }
...
RC Table::insert_record(Trx *trx, int value_num, const Value *values) {
  ...
  char *record_data;
  RC rc = make_record(value_num, values, record_data);  // <= G!
  if (rc != RC::SUCCESS) {
...
RC Table::make_record(int value_num, const Value *values, char *&record_out) {
  ...
  for (int i = 0; i < value_num; i++) {
    ...
    if (field->type() != value.type) {
      if(typecast_whz((Value *)&value,field->type()) ){
        continue;
      }else{
      LOG_ERROR(  // TO DO typecast
  ...
  memset(record, 0, table_meta_.field(normal_field_start_index)->offset());
  for (int i = 0; i < value_num; i++) {
    const FieldMeta *field = table_meta_.field(i + normal_field_start_index);
    const Value &value = values[i];
...
RC Table::change_record(int value_index, const Value *values,
                        char *&record_out) {
  // 已经做了类型检查了

  // char *record = record_out;
  const int normal_field_start_index = table_meta_.sys_field_num();
  const FieldMeta *field =
      table_meta_.field(value_index + normal_field_start_index);
...
RC Table::create_index(Trx *trx, const char *index_name, const std::vector<char *> attribute_name, const bool &is_unique)
{
  if (common::is_blank(index_name) || attribute_name.empty()) {
  ...
  for (auto attr_name: attribute_name) {
    FieldMeta * fm = (FieldMeta *)table_meta_.field(attr_name);
    if (!fm) {
      field_meta.clear();
      break;
    }
    field_meta.push_back(*fm);
  }
...
RC Table::update_record(Trx *trx, const Value *value, int value_index,
                        Record *record) {
  if (nullptr == value) {
    LOG_ERROR("Invalid value. value: %s", value);
    return RC::INVALID_ARGUMENT;
  }
  char *record_data;
  ;
  ...
  // int value_num=1;
  // 只更新一个字段的 别的字段全部复制即可
  // 创建record value->record_data
  // 这里对两个及以上的字段存在问题
  // RC rc = make_record(value_num, value, record_data);
  RC rc = change_record(value_index, value, record_data);
  if (rc != RC::SUCCESS) {
    LOG_ERROR("Failed to create a record. rc=%d:%s", rc, strrc(rc));
    return rc;
  }
...
RC Table::update_record(Trx *trx, Record *record) {
  ...
  rc = record_handler_->update_record(
      record);  // 这里也需要实现 ? 似乎不需要了 只需要传入正确的record
  if (rc != RC::SUCCESS) {
...
void Table::all_indexes(std::vector<Index *> &indexes) const {
  for (const auto &index_item : indexes_) {
    indexes.emplace_back(index_item);
  }
}
