...
void value_init_string(Value *value, const char *v, int len)
{
  // 对于char字符串分为两种情况 短的是char类型 长的赋予text类型
  if (len > 24) {
    // if(len>4096)
    value->type = TEXTS;
...
void inserts_init(
    Inserts *inserts, const char *relation_name, Value values[], size_t value_num, size_t value_list_length)
{
  ...
  inserts->value_insert_length=value_list_length;
}
...
void create_index_reset(CreateIndex *create_index)
{
  create_index->attribute_count = 0;
}

void create_index_append_attribute(CreateIndex *create_index,
...
void create_unique_index_init(
    CreateUniqueIndex *create_unique_index, const char *index_name, const char *relation_name, const char *attr_name)
{
  create_unique_index->index_name = strdup(index_name);
  create_unique_index->relation_name = strdup(relation_name);
  create_unique_index->attribute_name[create_unique_index->attribute_count++] = strdup(attr_name);
}

void create_unique_index_reset(CreateUniqueIndex *create_unique_index)
{
  create_unique_index->attribute_count = 0;
}

void create_unique_index_append_attribute(CreateUniqueIndex *create_unique_index, const char *attr_name)
{
  create_unique_index->attribute_name[create_unique_index->attribute_count++] = strdup(attr_name);
}

void create_unique_index_destroy(CreateUniqueIndex *create_unique_index)
{
  free(create_unique_index->index_name);
  free(create_unique_index->relation_name);
  for (size_t i = 0; i < create_unique_index->attribute_count; i++) {
    free(create_unique_index->attribute_name[i]);
  }
  // free(create_unique_index->attribute_name);

  create_unique_index->index_name = nullptr;
  create_unique_index->relation_name = nullptr;
  create_unique_index->attribute_count = 0;
  for (size_t i = 0; i < create_unique_index->attribute_count; i++) {
    create_unique_index->attribute_name[i] = nullptr;
  }
}
...
void query_reset(Query *query)
{
  switch (query->flag) {
    ...
    case SCF_CREATE_UNIQUE_INDEX: {
      create_unique_index_destroy(&query->sstr.create_unique_index);
    } break;
    case SCF_DROP_INDEX: {
